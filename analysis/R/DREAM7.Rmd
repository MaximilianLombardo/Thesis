...
ge <- read.table(file = "/Thesis/data/DREAM7/Train/DREAM7_DrugSensitivity1_GeneExpression.txt", header = TRUE)
row.names(ge) <- ge$HGNC_ID
ge <- ge[,-1]

meth <- read.table(file = "/Thesis/data/DREAM7/Train/DREAM7_DrugSensitivity1_Methylation.txt", header = TRUE)
row.names(meth) <- meth$HGNC_ID

meth <- meth[,c(-1, -2, -3, -4)]

rna <- read.table(file = "/Thesis/data/DREAM7/Train/DREAM7_DrugSensitivity1_RNAseq_quantification.txt", header = TRUE)

rownames(rna) <- rna$HGNC_ID

rppa <- read.table(file = "/Thesis/data/DREAM7/Train/DREAM7_DrugSensitivity1_RPPA.txt", header = TRUE)
